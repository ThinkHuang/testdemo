...
public class ThreadPoolUtils {
     ...
     */
    static {
        scheduleExec = Executors.newScheduledThreadPool(THREAD_SIZE);
        exec = new MyNewFixedThreadPool(THREAD_SIZE, THREAD_SIZE, 0, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<Runnable>());
     ...
     * @return
     */
    public static long getFixedTaskCount() {
        if (null == exec) {
            return 0L;
        }
        ThreadPoolExecutor fixed = (ThreadPoolExecutor)exec;
        return fixed.getTaskCount();
    }
    ...
    private static class MyNewFixedThreadPool extends ThreadPoolExecutor {
        
        public MyNewFixedThreadPool(int corePoolSize, int maximumPoolSize, long keepAliveTime, TimeUnit unit, BlockingQueue<Runnable> workQueue) {
    ...
    public static void main(String[] args) {
        Thread thread1 = new Thread(new Runnable() {
            ...
            @Override
            public void run() {
                System.out.println("测试线程异常1");
            }
        });
        ThreadPoolUtils.execute(thread1);
        
        Thread thread2 = new Thread(new Runnable() {
            
            @Override
            public void run() {
                throw new NullPointerException("测试线程异常2");
            }
        });
        
        ThreadPoolUtils.execute(thread2);
